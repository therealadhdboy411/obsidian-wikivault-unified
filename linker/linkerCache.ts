...
export class PrefixTree {
    static readonly BOUNDARY_REGEX = /[^\p{L}]/u;
    static readonly FORMATTING_REGEX = /[^\p{L}\p{N}]/u;
    ...
    getCurrentMatchNodes(index: number, excludedNote?: TFile | null): MatchNode[] {
        ...
        for (const node of this._currentNodes) {
            ...
            // Optimization: Avoid unnecessary Set and Array allocations when no note is excluded
            // or when the excluded note is not present in the files set.
            let files = node.node.files;
            if (excludedNote && files.has(excludedNote)) {
                files = new Set(Array.from(files).filter((file) => file.path !== excludedNote.path));
            }
            ...
            matchNode.files = files;
            matchNode.value = node.node.value;
            matchNode.requiresCaseMatch = node.node.requiresCaseMatch;

            const fileNames = Array.from(files).map((file) => file.basename);
            const nodeValue = node.node.value;
            matchNode.isAlias = !fileNames.map((n) => n.toLowerCase()).includes(nodeValue.toLowerCase());

            // Optimization: Removed redundant while loop that unnecessarily traversed trie parents.
            // The case matching status is already tracked in VisitedPrefixNode.
    ...
    private static isUpperCaseString(value: string | null | undefined, upperCasePart = 0.75) {
        ...
        return upperCaseChars / value.length >= upperCasePart;
    }
    ...
    pushChar(char: string) {
        ...
        // Boundary status is invariant under case conversion for our regex
        const isBoundary = PrefixTree.checkWordBoundary(char);

        // Use a Set to avoid O(N^2) behavior when checking for existing nodes in newNodes
        const addedNodes = new Set<PrefixNode>();

        const processChar = (c: string, isOriginalCase: boolean) => {
            if (this.settings.matchAnyPartsOfWords || isBoundary || this.settings.matchEndOfWords) {
                if (!addedNodes.has(this.root)) {
                    newNodes.push(new VisitedPrefixNode(this.root, true, isBoundary));
                    addedNodes.add(this.root);
                }
            }

            for (const node of this._currentNodes) {
                const child = node.node.children.get(c);
                if (child && !addedNodes.has(child)) {
                    // Optimization: Use O(1) Set lookup instead of O(N) Array.map().includes()
                    const newVisited = new VisitedPrefixNode(child, isOriginalCase, node.startedAtWordBeginning);
        ...
        // Process original character
        processChar(char, true);

        // If the character has a different lowercase version, process it too
        // This avoids double-processing for spaces, digits, and already-lowercase letters
        if (char !== lowerChar) {
            processChar(lowerChar, false);
        }

        // TODO: Ignore formatting (#59) - Kept for future compatibility
        /*
    ...
    static checkWordBoundary(char: string): boolean {
        return PrefixTree.BOUNDARY_REGEX.test(char);
    }

    static isFormattingChar(char: string): boolean {
        return PrefixTree.FORMATTING_REGEX.test(char);
    }
}
